...
type RegisterProps = {
  onChangeLogin?: () => void
  onRegister: (data: RegisterFormValues) => AuthResult
}
...
function Register({ onChangeLogin, onRegister }: RegisterProps) {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    // La validación de duplicados (correo/cédula ya registrados) vive en
    // App.tsx, que es quien tiene el registro de cuentas en memoria.
    const result = onRegister({ nombre, correo, cedula, password: formData.password })
    if (!result.ok) {
