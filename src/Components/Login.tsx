...
type LoginProps = {
  ...
  onLogin: (cedula: string, password: string) => Promise<AuthResult>
}
...
function Login({ onChangeRegister, onLogin }: LoginProps) {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setIsLoading(true)
    const result = await onLogin(trimmedCedula, password)
    setIsLoading(false)
      ...
      <section className="flex-1 bg-[#e5e7eb] flex flex-col items-center justify-center px-8 py-12 md:px-24">
        <div className="w-full max-w-sm">
          ...
          <div className="text-center text-sm text-gray-700 font-medium">
            Crear una cuenta{' '}
            <button type="button" className="text-amber-500 hover:underline" onClick={onChangeRegister}>
              aquí
            </button>
          </div>
