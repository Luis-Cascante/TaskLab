...
function SelectedTask({
  ...
  const isOwnTask = !!currentUserId && currentUserId === Task.employer_id
  const alreadyApplied = appliedTaskIds.includes(Task.id)
  const applyLabel = alreadyApplied ? "Ya aplicaste" : "Aplicar"

  return (
    <main className="flex-1 max-w-5xl w-full mx-auto p-4 md:p-6 flex flex-col gap-6">
      <div className="text-center py-2">
        <h1 className="text-2xl font-bold text-gray-800">Trabajos Publicados</h1>
      </div>
      ...
      <div className="bg-[#f59e0b] rounded-2xl shadow-lg flex flex-col">
        ...
        <div className="bg-[#1d61a1] text-white rounded-b-2xl p-6 md:p-8 flex flex-col md:flex-row gap-6 shadow-inner">
          ...
          <div className="flex-1 flex flex-col justify-between gap-4">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-xs md:text-sm font-semibold text-gray-900 bg-white/20 w-fit px-3 py-1 rounded-md">
                ...
                <span
                  onClick={() => onOpenProfile?.(Task.employer.id)}
              ...
              <div className="text-sm space-y-1.5 mt-2">
                <p className="leading-relaxed text-gray-100 font-light text-justify">
                  <strong className="text-gray-900 font-bold">Descripcion:</strong> {Task.description ?? "Sin descripción"}
                </p>
                <p className="text-gray-100"><strong className="text-gray-900 font-bold">Ubicacion:</strong> {Task.location}</p>
                <p className="text-gray-100"><strong className="text-gray-900 font-bold">Categoria:</strong> {Task.category.name}</p>
                <p className="text-gray-100"><strong className="text-gray-900 font-bold">Contrato:</strong> {Task.agreement.name}</p>
              </div>
            </div>
            ...
            <div className="flex justify-end gap-3 mt-4">
              {!isOwnTask && (
                <button
                  type="button"
                  onClick={() => !alreadyApplied && onApply?.(Task.id)}
                  disabled={alreadyApplied}
                  className={`font-black text-base px-8 py-2.5 rounded-xl shadow-md transition-colors tracking-wide
                    ${alreadyApplied
                      ? "bg-gray-300 text-gray-500 cursor-not-allowed"
                      : "bg-[#f59e0b] hover:bg-[#e08e06] text-gray-900 cursor-pointer"
                    }`}
                >
                  {applyLabel}
                </button>
